...
export default function TimeAndDate({
  ...
  useEffect(() => {
    const timer = setInterval(() => { // Запускаем "часы", чтобы они обновлялись каждую секунду.
  ...
  const handleClickOutside = useCallback((event: MouseEvent): void => {
    if (containerRef.current && !containerRef.current.contains(event.target as Node)) { // Если нажали мышкой *вне* календаря...
      setIsCalendarOpen(false); // ...закрываем календарь!
      setSelectedDate(new Date(dateTime)); // ...и сбрасываем выбранную дату на текущую.
    }
  }, [containerRef, dateTime]); // Этот код будет меняться, только если изменится "containerRef" или "dateTime".
  ...
  const formattedDate = useMemo(() => {
    return dateTime.toLocaleDateString(dateFormat, { // Превращаем дату в красивую строчку, используя формат, который нам дали родители.
  ...
  const formattedTime = useMemo(() => {
    return dateTime.toLocaleTimeString(timeFormat, { // Превращаем время в красивую строчку, используя формат, который нам дали родители.
  ...
  const handleDateChange = useCallback((date: Date | null): void => {
    if (date) { // Если мы действительно выбрали дату (а не отменили выбор)...
      setSelectedDate(date); // ...запоминаем эту дату.
      onDateChange?.(date); // ...и сообщаем об этом родителям, если они просили.
    }
  }, [onDateChange]); // Этот код будет меняться, только если изменится "onDateChange".
  ...
  const dayClassName = useCallback((date: Date): string => {
    ...
    const isToday = getYear(date) === getYear(today) && // Проверяем, является ли это сегодняшний день...
  ...
  return (
    <div className={styles.calendar_container} ref={containerRef} style={containerStyle}> {/* Весь календарь.  Как рама для картины. */}
      <button className={styles.TimeDate} onClick={toggleCalendar} aria-label="Открыть календарь" style={dateStyle}> {/* Кнопка, на которую мы нажимаем, чтобы открыть календарь. */}
        <div className={styles.date}>{formattedDate}</div> {/* Дата, которая отображается на кнопке. */}
      </button>
      <span className={styles.TimeNow} style={timeStyle}> {/* Место для показа времени. */}
        <div className={styles.time}>{formattedTime}</div> {/* Время, которое отображается. */}
      </span>
      {isCalendarOpen && ( // Если календарь открыт...
        <div className={`${styles.datePickerContainer} ${styles.fadeIn}`} role="dialog"> {/* ...показываем его! */}
          <DatePicker
